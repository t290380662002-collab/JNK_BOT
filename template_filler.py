...
def _merge_special_request_cell(ws, cell):
    """把特別要求輸入格橫向合併數欄，使文字落在表格格子內並置中。"""
    row = cell.row
    start_col = cell.column
    # 向右合併 3 格（共 4 欄寬），避免文字溢出表格線；若已遇到資料/合併格則提前停止
    end_col = start_col
    max_col = min(start_col + 3, 20)
    for c in range(start_col + 1, max_col + 1):
        if ws.cell(row=row, column=c).value is not None:
            break
        if _merged_range_of(ws, row, c) is not None:
            break
        end_col = c
...
def _fill_form_sheet(ws, b: dict):
    ...
    for field, subs in HT.FORM_LABELS.items():
        ...
        # 特別要求欄：把右側連續空單元格合併成一個大格，讓「吸菸/禁煙」顯示在格子內
        if field == "special_request" and v:
            _merge_special_request_cell(ws, cell)
